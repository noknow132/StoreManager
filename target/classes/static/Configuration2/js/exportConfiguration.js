...
//加载出口
function loadOutLet(){
	$.get("../outLet/searchOutLet",function(result){
		$("#storeHourse_ul_id").empty();
		for (var i = 0; i < result.length; i++) {
			$("#storeHourse_ul_id").append('<li class="config_message_table_content">'
			+'<div><span class="ml30">'+result[i].outletName+'</span></div>'		
			+'<div>'+(result[i].outType=="0"?"出货口":"分拣口")+'</div>'
			+'<div>'+result[i].outNo+'</div>'
			+'<div>'			
			+(result[i].isUesd=='0'?'<img class="config_table_diss ml5" src="../image/diss.png"/>':'<img class="config_table_start  ml5" src="../image/start.png"/>')		
			+'</div>'		
			...
			+'<span style="color: #078fff;" class="fl config_disable" dataNo="'+result[i].outletId+'">'+(result[i].isUesd=='0'?"启用":"禁用")+'</span>'		
			+'<span style="color: #078fff;" class="fl config_alert ml20" dataNo="'+result[i].outletId+'">修改</span>'		
			+'</div>'		
			+'</li>');
		}
		//修改出口
